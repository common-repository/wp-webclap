 ...
 * </table>
 */
class Wp_WebClap_View extends Wp_WebClap_Base
{
    /**
     * テンプレートディレクトリを保持する
     * @var string
    ...
    {
        $this->m_templatedir = realpath(WP_WEBCLAP_DIR_ROOT . '/templates');
        $this->m_cachedir = realpath(WP_WEBCLAP_DIR_ROOT . '/caches');
    }
    ...
    public function render($template, array $values)
    {
        ...
        return $this->display($cfile, $values);
    }
    ...
    private function structureReplace($buffer, array $values)
    {
        // テンプレートの継承
        $isExtend = false;
        $extends = '<?php $v=new Wp_WebClap_View(); echo $v->render("%s", $values); ?>';
        $blockStart = '';
        $blockEnd = '';
        $regexp = '/\{% extends *\((\w+)\) %\}/';
        ...
        $reppattern = array(
            '/\{% block (\w+) %\}/' => $blockStart,
            '/\{% endblock (\w+) %\}/' => $blockEnd,
            '/\{% __\(([^}]+)\) %\}/' => '<?php echo  __($1, "' . self::TEXTDOMAIN . '"); ?>',
